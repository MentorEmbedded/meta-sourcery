...
PROVIDES += " \
	eglibc-source-dbg \
        ...
        ${@base_conditional('PREFERRED_PROVIDER_linux-libc-headers', PN, 'virtual/linux-libc-headers', '', d)} \
        libgomp \
        libgomp-staticdev \
        libgomp-dev \
        libgomp-dbg \
...
PACKAGES = "eglibc-source-dbg gdb gdb-dbg gdbserver gdbserver-dbg \
             ...
             ${@base_conditional('PREFERRED_PROVIDER_linux-libc-headers', PN, 'linux-libc-headers', '', d)} \
             ${@base_conditional('PREFERRED_PROVIDER_linux-libc-headers', PN, 'linux-libc-headers-dev', '', d)} \
...
DO_HEADERS = "${@base_conditional('PREFERRED_PROVIDER_linux-libc-headers', PN, 'true', 'false', d)}"
do_install() {
	install -d ${D}${bindir}
	...
	if ${DO_HEADERS}; then
		...
		kernel_headers=$(echo " ${FILES_${WRL_TOOLCHAIN_VARIANT}linux-libc-headers-dev}" | sed -e 's/[\x09]/ /g' -e 's/  */ X/g')
